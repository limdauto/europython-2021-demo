...
with DAG(
    ...
    tasks["clean-movies-raw-movies-cleaned-movies"] = KedroOperator(
        ...
        node_name="clean_movies([raw_movies]) -> [cleaned_movies]",
    ...
    tasks["clean-ratings-raw-ratings-cleaned-ratings"] = KedroOperator(
        ...
        package_name=package_name,
        pipeline_name=pipeline_name,
        node_name="clean_ratings([raw_ratings]) -> [cleaned_ratings]",
        project_path=project_path,
        env=env,
    )

    tasks["create-model-input-table-cleaned-movies-cleaned-ratings-rated-movies"] = KedroOperator(
        task_id="create-model-input-table-cleaned-movies-cleaned-ratings-rated-movies",
        package_name=package_name,
        pipeline_name=pipeline_name,
        node_name="create_model_input_table([cleaned_movies,cleaned_ratings]) -> [rated_movies]",
        project_path=project_path,
        env=env,
    )
    ...
    tasks["evaluate-model"] = KedroOperator(
    ...
    tasks["clean-movies-raw-movies-cleaned-movies"] >> tasks["create-model-input-table-cleaned-movies-cleaned-ratings-rated-movies"]

    tasks["clean-ratings-raw-ratings-cleaned-ratings"] >> tasks["create-model-input-table-cleaned-movies-cleaned-ratings-rated-movies"]
    ...
    tasks["create-model-input-table-cleaned-movies-cleaned-ratings-rated-movies"] >> tasks["split-data"]
